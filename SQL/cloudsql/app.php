...
$app->get('/', function () use ($app) {
    ...
    $ress = $db->query('SELECT image from 6thsense ORDER BY image_datetime desc LIMIT 10');
    $images = array();
	foreach($ress as $key => $res){
		$images[$key] = base64_encode( $res['image'] );
	}
	$results = $db->query('SELECT security_id,image_datetime from 6thsense ORDER BY image_datetime desc LIMIT 10');
